...
- **Try with sample data:** on first visit (no habits yet), a one-time prompt offers **Try sample** or **Skip**. Anytime later: Settings → **Load sample** (~12 weeks of fake habits ending today; replaces this browser’s data after confirm). After a first-visit sample load, a tip offers **Reset all** or continue. **Reset all** clears this browser and, if signed in, overwrites the Drive file with empty data, then opens the habit picker. Export first if you want a backup.
